...
export type BankTemplateConfig = {
  ...
  /**
   * Consolidation Protected Vocabularies
   *
   * Closed vocabularies whose terms are mutually exclusive during consolidation: [{"name": "environment", "terms": ["staging", "production"]}]
   */
  consolidation_protected_vocabularies?: Array<{
    [key: string]: unknown;
  }> | null;
  /**
...
export type KnowledgePageResponse = {
   ...
   * Is Stale
   *
   * True when a memory in this page's scope has been written since the page last read the memories — the same check the refresh gate asks, and the one the tree endpoint already reports. Null when the page has no backing mental model.
   */
  is_stale?: boolean | null;
};
