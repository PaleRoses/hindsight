...
from .llm_wrapper import LLMConfig, sanitize_llm_output, sanitize_text
...
def _profile_route_to_llm(
    ...
    """Construct one named-profile route with the existing provider implementation."""
    ...
    return LLMConfig(
        ...
        model=route.model,
        reasoning_effort=route.reasoning_effort or DEFAULT_LLM_REASONING_EFFORT,
        ...
        groq_service_tier=route.groq_service_tier or DEFAULT_LLM_GROQ_SERVICE_TIER,
        ...
        timeout=route.timeout if route.timeout is not None else DEFAULT_LLM_TIMEOUT,
        max_retries=route.max_retries if route.max_retries is not None else DEFAULT_LLM_MAX_RETRIES,
        initial_backoff=(route.initial_backoff if route.initial_backoff is not None else DEFAULT_LLM_INITIAL_BACKOFF),
        max_backoff=route.max_backoff if route.max_backoff is not None else DEFAULT_LLM_MAX_BACKOFF,
    )
...
class MemoryEngine(MemoryEngineInterface):
    ...
    def __init__(
        ...
        # LLMProvider applies the canonical provider base URL when this is empty.
        memory_llm_base_url = memory_llm_base_url or ""
    ...
    async def _bank_operation_scope(
        ...
        *,
        ...
        try:
            ...
            try:
                yield replace(resolved_config) if resolved_config is not None else None
            finally:
    ...
    async def _resolve_full_config(
        ...
        if (
            ...
            return replace(operation.config)
        ...
        return replace(resolved_config)
    ...
    def _inference_llm(self, operation: str) -> "LLMConfig | MultiLLMProvider":
        ...
        profile_id = (
            operation_state.config.inference_profile
