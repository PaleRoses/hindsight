...
class HindsightConfig:
    ...
    reflect_llm_reasoning_effort: str | None  # Effort for reflect calls; falls back to llm_reasoning_effort
    ...
    _CREDENTIAL_FIELDS = {
        ...
        # Named profile registry routes contain resolved API keys and private endpoints.
        "inference_profiles",
    ...
    def from_env(cls) -> "HindsightConfig":
        ...
        config = cls(
            ...
            llm_reasoning_effort=(
                os.getenv(ENV_LLM_REASONING_EFFORT, DEFAULT_LLM_REASONING_EFFORT)
                if legacy_routing
                else DEFAULT_LLM_REASONING_EFFORT
            ),
            llm_groq_service_tier=(
                os.getenv(ENV_LLM_GROQ_SERVICE_TIER, DEFAULT_LLM_GROQ_SERVICE_TIER)
                if legacy_routing
                else DEFAULT_LLM_GROQ_SERVICE_TIER
            ),
            llm_openai_service_tier=(
                os.getenv(ENV_LLM_OPENAI_SERVICE_TIER, DEFAULT_LLM_OPENAI_SERVICE_TIER)
                if legacy_routing
                else DEFAULT_LLM_OPENAI_SERVICE_TIER
            ),
            llm_bedrock_service_tier=(
                os.getenv(ENV_LLM_BEDROCK_SERVICE_TIER) or None if legacy_routing else DEFAULT_LLM_BEDROCK_SERVICE_TIER
            ),
            ...
            # A selected named profile owns all four routes and never parses or
            # inherits the unrelated legacy multi-LLM fallback chains.
