...
| `HINDSIGHT_API_CONSOLIDATION_PROTECTED_VOCABULARIES` | Optional JSON array of closed vocabularies whose terms are mutually exclusive in one observation, for example `[{"name":"environment","terms":["staging","production"]}]`. Hindsight NFKC-normalizes and case-folds terms, then rejects known cross-term CREATEs, UPDATEs, semantic-dedup candidates, and merged text. It retries without the conflicting target and fails the batch closed after `CONSOLIDATION_MAX_ATTEMPTS`; text containing no known term remains permissive. Vocabulary names must be unique and each vocabulary needs at least two unique terms. Configurable per bank. | `[]` |
